...
export class StepExecutor {
    ...
    while (true) {
      ...
      try {
        ...
        if (this.screenshotOnFailure) {
          try {
            const screenshotPath = await this.pageClient.screenshot(`failure_${step.id}`);
            ...
            this.context.logger.warn(
              ...
              'Failed to capture failure screenshot (extension-side screenshot not yet implemented — task 8)',
            );
          }
        }
   ...
   *      same selector strings the original code produced.
   *
   * The LLM fallback path also used `page.$(selector)` to validate each
   * LLM-suggested selector. We replace that with `pageClient.elementExists()`,
   * which is semantically identical (returns true/false).
   */
  private async discoverItems(itemsConfig: LoopItems, maxItems: number): Promise<string[]> {
    if (itemsConfig.selectorPattern) {
      try {
        const count = await this.pageClient.countMatching(itemsConfig.selectorPattern);
        if (count > 0) {
          const selectors = Array.from({ length: Math.min(count, maxItems) }, (_, idx) =>
    ...
    for (const item of result.items) {
      try {
        const exists = await this.pageClient.elementExists(item.selector);
  ...
  private async runDeterministicCheck(
    ...
    switch (check) {
      case 'element_exists':
        return this.pageClient.elementExists(value);
      case 'element_missing':
        return !(await this.pageClient.elementExists(value));
      case 'url_matches':
        return (await this.pageClient.getUrl()).includes(value);
      case 'text_contains':
        return (await this.pageClient.getHtml()).includes(value);
  ...
  private async executeNavigate(step: Step): Promise<void> {
    ...
    const t0 = Date.now();
    await this.pageClient.navigate(url);
    this.context.logger.debug(
  ...
  private async executeInteract(step: Step): Promise<void> {
    ...
    switch (action.interaction) {
      ...
      case 'type': {
        ...
        await this.pageClient.type(selector, text);
        break;
      }
      ...
      case 'select': {
        ...
        await this.pageClient.selectOption(selector, value);
        break;
      }

      case 'check':
        await this.pageClient.check(selector);
        break;

      case 'uncheck':
        await this.pageClient.uncheck(selector);
        break;

      case 'hover':
        await this.pageClient.hover(selector);
        break;

      case 'focus':
        await this.pageClient.focus(selector);
        break;
  ...
  private async executeWait(step: Step): Promise<void> {
    ...
    // PageClient fall back to its own internal default. This is what
    ...
    switch (action.condition) {
      case 'selector': {
        ...
        await this.pageClient.waitForSelector(selector, effectiveTimeout);
        break;
      }

      case 'navigation':
        await this.pageClient.waitForNavigation(action.value, effectiveTimeout);
        break;
      ...
      case 'network_idle':
        await this.pageClient.waitForNetworkIdle(effectiveTimeout);
        break;
  ...
  private async executeExtract(step: Step): Promise<void> {
    ...
    switch (action.target) {
      case 'text': {
        ...
        value = await this.pageClient.getText(selector);
        break;
      }
      ...
      case 'attribute': {
        ...
        value = await this.pageClient.getAttribute(selector, action.attribute);
        break;
      }
      ...
      case 'title':
        value = await this.pageClient.getTitle();
        break;

      case 'screenshot': {
        // Screenshot is treated as an artifact; store path in outputs too.
        // The screenshot extension command is implemented in task 8; until
        // then, this will throw and be handled by the onFailure policy.
        const screenshotPath = await this.pageClient.screenshot(action.outputName);
        this.context.addArtifact(screenshotPath);
  ...
  //   5. Dispatch the chosen action through PageClient. If the action
  ...
  /**
   * Dispatch a single LLM-chosen action through PageClient. Bad
   * selectors, missing values, and extension errors all throw — the
  ...
  private async dispatchAiScopeAction(decision: NextActionResult): Promise<void> {
    ...
    switch (action) {
      case 'navigate':
        ...
        await this.pageClient.navigate(value);
        return;
      case 'click':
      ...
      case 'scroll': {
        ...
        await this.pageClient.scroll(direction);
        return;
      }
      case 'wait': {
   ...
   *
   * Callers narrow `action.successCheck` to non-undefined before invoking
   * this method — self-terminating mode skips the call entirely.
   */
  ...
  private async validateStep(step: Step): Promise<void> {
    ...
    switch (type) {
      case 'url_contains': {
        const url = await this.pageClient.getUrl();
      ...
      case 'element_visible': {
        const exists = await this.pageClient.elementExists(value);
        if (!exists) {
          throw new Error(
      ...
      case 'text_present': {
        const html = await this.pageClient.getHtml();
      ...
      case 'title_contains': {
        const title = await this.pageClient.getTitle();
