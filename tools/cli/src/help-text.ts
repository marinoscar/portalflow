...
export function topLevelHelpText(): string {
  return assemble([
    section('Description:', [
      ...
      'in a real Chrome browser via a Chrome extension over a local WebSocket,',
      'with LLM assistance for resilient element detection and decision-making',
      'when selectors fail.',
      '',
...
export function runHelpText(): string {
  return assemble([
    section('Description:', [
      'Execute an automation defined in a JSON file. The Chrome extension transport',
      'drives the browser over a local WebSocket — no Playwright dependency.',
      '',
    ...
    section('Notes:', [
      ...
      '- Automations may declare reusable functions in a top-level "functions"',
      '  section and invoke them via the "call" step type. Conditions can branch',
      '  into a function via "thenCall" / "elseCall". See',
      '  docs/AUTOMATION-JSON-SPEC.md#45-functions for the full reference.',
    ]),
    section('Exit codes:', [
...
export function validateHelpText(): string {
  return assemble([
    ...
    section('Schema (top-level fields):', [
      ...
      'steps[]      Array of step objects (navigate, interact, wait, extract, tool_call, condition, download, loop, call)',
      'functions[]  Optional. Reusable named blocks of steps invoked via the "call" step type',
...
export function settingsVideoHelpText(): string {
  return assemble([
    section('Description:', [
      ...
      '',
      'Video recording captures the browser session and saves files as .webm.',
      'Videos are written to the configured video directory only after the',
      'browser context closes cleanly — if the process is killed mid-run,',
...
export function settingsBrowserHelpText(): string {
  return assemble([
    section('Description:', [
      ...
      '',
      '  isolated   — Default. A fresh in-memory browser context for every run.',
      '               Cookies, extensions, and sign-in state do NOT persist',
      '               between runs.',
      '',
      '  persistent — Opens a real on-disk Chrome profile. Cookies, localStorage,',
      '               extensions, saved logins, and history all persist between',
      '               runs. Recommended for portals that fingerprint bots or that',
      '               depend on long-lived sessions.',
      '',
