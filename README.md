...
### PortalFlow2 CLI — browser automation via Chrome extension (recommended)

The [`@portalflow/cli`](tools/cli/README.md) package (`portalflow` binary) is the current-generation automation runtime. It executes automation JSON files by sending commands to the PortalFlow Chrome extension over a local WebSocket, eliminating every Playwright-based browser-launch failure mode catalogued in the architectural retrospective at [`docs/BROWSER-CONTROL-STRATEGY.md`](docs/BROWSER-CONTROL-STRATEGY.md). One-time setup: build the schema and the extension (`npm -w tools/schema run build && npm -w tools/extension run build`), load `tools/extension/dist/` unpacked in Chrome, then build and run the CLI (`npm -w tools/cli run build && portalflow`). See [`tools/cli/README.md`](tools/cli/README.md) for the full install guide, command reference, protocol documentation, and troubleshooting.

### Chrome extension — record, edit, and export automations

The [`@portalflow/extension`](tools/extension/README.md) package is a Manifest V3 Chrome extension that records browser workflows, lets you edit them in a side panel (with optional LLM assist), and exports them as JSON files that the CLI can run unmodified. It captures HTML snapshots at each event, preserves the raw original recording, tracks a version history with undo/redo, supports conversational AI edits with explicit approval, and round-trips entire sessions via a zip format documented at [`docs/EXTENSION-SESSION-FORMAT.md`](docs/EXTENSION-SESSION-FORMAT.md). Auto-detects password and OTP fields and wires them to `vaultcli` and `smscli` respectively. See [`tools/extension/README.md`](tools/extension/README.md) for installation and usage.

### Coding-agent friendly

**Coding-agent friendly** — `--json` output, stable exit codes, schema and tools-list introspection, plus a bundled OpenClaw skill. See [AGENT-INTEGRATION.md](docs/AGENT-INTEGRATION.md) and [OPENCLAW-INTEGRATION.md](docs/OPENCLAW-INTEGRATION.md).

**Goal-driven mode** — `portalflow agent "<your goal>"` runs a one-step agent automation without authoring JSON. See [AGENT-INTEGRATION.md](docs/AGENT-INTEGRATION.md#goal-driven-mode-portalflow-agent).
